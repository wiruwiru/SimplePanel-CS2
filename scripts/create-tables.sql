...
CREATE TABLE IF NOT EXISTS `sp_ban_comments` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_ban_comment_ban` (`ban_id`),
  KEY `idx_admin_steamid` (`admin_steamid`),
  CONSTRAINT `fk_ban_comment_ban` FOREIGN KEY (`ban_id`) REFERENCES `sa_bans` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

CREATE TABLE IF NOT EXISTS `sp_mute_comments` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_mute_comment_mute` (`mute_id`),
  KEY `idx_admin_steamid` (`admin_steamid`),
  CONSTRAINT `fk_mute_comment_mute` FOREIGN KEY (`mute_id`) REFERENCES `sa_mutes` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
('@web/ban.comment.add', 'Web-only: Permission to add comments to bans.', 0),
('@web/ban.comment.delete', 'Web-only: Permission to delete any ban comment.', 0),
('@web/ban.comment.delete.own', 'Web-only: Permission to delete own ban comments.', 0),

('@web/mute.comment.add', 'Web-only: Permission to add comments to mutes.', 0),
('@web/mute.comment.delete', 'Web-only: Permission to delete any mute comment.', 0),
('@web/mute.comment.delete.own', 'Web-only: Permission to delete own mute comments.', 0)
