...
import { getSessionFromCookies, verifySessionToken } from "@/utils/session"
...
export async function getUserFlags(steamId) {
  try {
    ...
    const panelAdmin = process.env.PANEL_ADMIN
    if (panelAdmin && panelAdmin.trim() !== "" && steamId === panelAdmin.trim()) {
      return ["@web/root"]
    }
...
export async function verifyAdminAccess(req, requiredFlag = null) {
  try {
    ...
    const panelAdmin = process.env.PANEL_ADMIN
    if (panelAdmin && panelAdmin.trim() !== "" && user.steamId === panelAdmin.trim()) {
      return { authorized: true, flags: ["@web/root"] }
    }
