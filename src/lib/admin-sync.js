...
export async function syncServerGroupAdmins(serverGroupId, oldServerIds = [], newServerIds = []) {
  try {
    ...
    for (const admin of allAdmins) {
      ...
      // Check if admin has AT LEAST the servers of the group (superset)
      const hasAllOldServers = oldServerIdsSorted.length > 0 && oldServerIdsSorted.every(id => adminServerIds.includes(id))
      
      if (oldServerIdsSorted.length > 0 && hasAllOldServers) {
        adminsInGroup.push({
          steamId: steamId,
    ...
    for (const admin of adminsInGroup) {
      ...
      if (removedServers.length > 0) {
        const isStrictMatch = admin.currentServerIds.length === oldServerIdsSorted.length
        if (isStrictMatch) {
          await db.query(
...
export async function syncAllServersAdmins(newServerIds = null) {
  if (newServerIds !== null && !Array.isArray(newServerIds)) {
  ...
  try {
    ...
    for (const admin of allAdmins) {
      ...
      if (adminServerIds.length > 0) {
        ...
        const hasAllExceptNew = newServerIds && 
          adminServerIds.length === allServerIds.length - newServerIds.length && 
          newServerIds.every(id => !adminServerIds.includes(id)) && 
          allServerIds.filter(id => !newServerIds.map(nid => Number(nid)).includes(id)).every(id => adminServerIds.includes(id))
    ...
    for (const admin of allServersAdmins) {
      ...
      const serversToCheck = newServerIds ? newServerIds : allServerIds
