...
export function MuteForm({ open, onOpenChange, editingMute, formData, setFormData, onSubmit, onCancel, isLoading = false }) {
  const { t } = useI18n()
  return (
      ...
      <DialogContent className="bg-zinc-900 border-zinc-800 text-zinc-100">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select id="type" value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} disabled={isLoading}>
          ...
          <div className="space-y-2">
            ...
            <Textarea id="reason" placeholder={t('mutes.reason_placeholder')} value={formData.reason} onChange={(e) => setFormData({ ...formData, reason: e.target.value })} className="bg-zinc-800 border-zinc-700 text-zinc-100" required disabled={isLoading} />
          </div>
          <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button type="submit" className="hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed" style={{ backgroundColor: 'var(--theme-primary)', color: 'var(--theme-primary-foreground)' }} onMouseEnter={(e) => { if (!isLoading) { e.currentTarget.style.opacity = '0.9'; e.currentTarget.style.backgroundColor = 'var(--theme-primary-hover)'; } }} onMouseLeave={(e) => { if (!isLoading) { e.currentTarget.style.opacity = '1'; e.currentTarget.style.backgroundColor = 'var(--theme-primary)'; } }} disabled={isLoading}>
              {isLoading ? (
                ...
                editingMute ? t('mutes.save_changes') : t('mutes.create_mute')
              )}
            </Button>
          </DialogFooter>
        </form>
