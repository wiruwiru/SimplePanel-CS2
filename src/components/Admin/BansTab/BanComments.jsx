...
export function BanComments({ banId }) {
  const { t } = useI18n()
  ...
  const apiPath = '/api/admin/sanctions/bans/comments'

  const fetchComments = useCallback(async () => {
    if (!banId) return
    try {
      setLoading(true)
      const response = await fetch(`${apiPath}?banId=${banId}`, { cache: 'no-store' })
      if (response.ok) {
  ...
  const addPermission = '@web/ban.comment.add'
  const deletePermission = '@web/ban.comment.delete'
  ...
  return (
    <div className="mt-3 pt-3 border-t border-zinc-700" data-ban-id={banId}>
      ...
      {showAddForm && (
        <div className="mb-3 p-3 bg-zinc-900 rounded border border-zinc-700">
          ...
          <div className="flex gap-2 mt-2">
            ...
            <Button size="sm" variant="outline" onClick={() => { setShowAddForm(false); setNewComment('') }} className="bg-zinc-900 border-zinc-700 text-zinc-300 hover:bg-zinc-700">
          ...
          <div key={comment.id} className="p-2.5 bg-zinc-900 rounded border border-zinc-700">
            {editingId === comment.id ? (
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <Button size="sm" onClick={() => handleEditComment(comment.id)} disabled={!editComment.trim() || submitting} className="bg-blue-600 hover:bg-blue-700 text-white">{t('common.save') || 'Save'}</Button>
                  <Button size="sm" variant="outline" onClick={cancelEdit} className="bg-zinc-900 border-zinc-700 text-zinc-300 hover:bg-zinc-700">{t('common.cancel') || 'Cancel'}</Button>
                </div>
              </div>
            ) : (
              <>
