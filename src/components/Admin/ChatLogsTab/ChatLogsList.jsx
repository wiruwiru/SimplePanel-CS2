...
const getTeamConfig = (team, t) => {
  switch (team) {
    ...
    default:
      return { label: t('servers.unknown_team'), className: "bg-zinc-700 text-white" }
  }
}

const TeamBadge = ({ team, t }) => {
  const config = getTeamConfig(team, t)
  return (
...
export function ChatLogsList({ chatlogs, loading, getAvatarUrl, getDisplayName, currentPage, totalPages, startIndex, total, onPageChange }) {
  ...
  if (chatlogs.length === 0) {
    return (
      <div className="text-center py-8 text-zinc-400">{t('servers.no_messages_found')}</div>
    )
  }
