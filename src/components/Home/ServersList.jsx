...
export function ServersList() {
  ...
  const fetchServerDetails = async (serverId, address) => {
    ...
    try {
      ...
      setDetailedInfo((prev) => ({ 
        ...prev, 
        [serverId]: { offline: true, error: t('servers.connection_error') } 
      }))
    } finally {
  ...
  const handleCopyIP = async (serverId, address) => {
    try {
      ...
      addToast({title: t('servers.connection_command_copied'), color: "success", variant: "solid"})
      setTimeout(() => setCopiedId(null), 2000)
    } catch (error) {
      console.error("Error copying to clipboard:", error)
      addToast({title: t('servers.copy_error'), color: "danger", variant: "solid"})
    }
  }
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-zinc-100 text-lg md:text-xl">
            <Server className="size-5" style={{ color: 'var(--theme-primary)' }} />{t('admin.tabs.servers')}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-zinc-400 text-center py-4">{t('servers.no_servers_available')}</p>
        </CardContent>
      </Card>
    )
  }
