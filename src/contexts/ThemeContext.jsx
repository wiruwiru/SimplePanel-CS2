...
export function ThemeProvider({ children }) {
  ...
  const [configDefaultTheme, setConfigDefaultTheme] = useState("default");
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (config.defaultTheme) {
            setConfigDefaultTheme(config.defaultTheme);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedTheme = localStorage.getItem("theme") || configDefaultTheme;
      setTheme(savedTheme);
      applyTheme(savedTheme);
    }
  }, [applyTheme, configDefaultTheme]);
