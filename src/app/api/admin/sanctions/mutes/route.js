  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO sa_mutes (player_name, player_steamid, admin_name, admin_steamid, reason, duration, ends, created, status, type)
       VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), 'ACTIVE', ?)`,
      [finalPlayerName, playerSteamId, adminName, user.steamId, reason, durationMinutes, ends, type || 'GAG']
    )
    ...
    try {
      ...
      console.error("Error sending webhook (mute created anyway):", webhookError)
    }
...
export async function PATCH(request) {
  try {
    ...
    if (status === 'UNMUTED') {
      ...
      if (!canUnmute) {
        return NextResponse.json(
          { error: "Access denied - You don't have permission to unmute this mute" },
          { status: 403 }
        )
      }
    } else {
      ...
      if (!canEdit) {
        return NextResponse.json(
          { error: "Access denied - You don't have permission to edit this mute" },
          { status: 403 }
        )
      }
    }
    ...
    if (status === 'UNMUTED') {
      if (existingMute[0].unmute_id) {
        return NextResponse.json(
          { error: "This mute has already been unmuted" },
          { status: 400 }
        )
      }
    ...
    if (status === 'UNMUTED' && unmuteReason) {
      newMuteData.unmuteReason = unmuteReason.trim()
    }
    ...
    try {
      ...
      console.error("Error sending webhook (mute updated anyway):", webhookError)
    }
...
export async function DELETE(request) {
  try {
    ...
    if (!canRemove) {
      return NextResponse.json(
        { error: "Access denied - You don't have permission to delete this mute" },
        { status: 403 }
      )
    }
    ...
    try {
      ...
      console.error("Error sending webhook (mute deleted anyway):", webhookError)
    }
