import { NextResponse } from "next/server"
import { cookies } from "next/headers"
import { db } from "@/lib/database"
import { verifyAdminAccess, getUserFlags } from "@/lib/api-auth"
import { hasPermission } from "@/utils/permissions"
import { sendDiscordWebhook } from "@/services/notifications/discord"
import { getAuthenticatedUser, convertBigIntToString } from "@/utils/api-helpers"

export async function GET(request) {
  try {
    const cookieStore = await cookies()
    const { user, error: userError, status: userStatus } = getAuthenticatedUser(cookieStore)
    
    if (userError) {
      return NextResponse.json({ error: userError }, { status: userStatus })
    }

    const { authorized, error: authError, status: authStatus } = await verifyAdminAccess(
      { headers: { cookie: `session=${cookieStore.get('session').value}` } },
      "@web/mute.view"
    )
    if (!authorized) {
      return NextResponse.json(
        { error: authError },
        { status: authStatus }
      )
    }

    const { searchParams } = new URL(request.url)
    const muteId = searchParams.get("muteId")

    if (!muteId) {
      return NextResponse.json(
        { error: "Mute ID is required" },
        { status: 400 }
      )
    }

    const comments = await db.query(
      `SELECT
        c.id,
        c.mute_id,
        c.admin_steamid,
        c.admin_name,
        c.comment,
        DATE_FORMAT(c.created_at, '%Y-%m-%dT%H:%i:%s') as created_at,
        DATE_FORMAT(c.updated_at, '%Y-%m-%dT%H:%i:%s') as updated_at
      FROM sp_mute_comments c
      WHERE c.mute_id = ?
      ORDER BY c.created_at ASC`,
      [muteId]
    )

    const formattedComments = (comments || []).map(comment => ({
      ...comment,
      created_at: comment.created_at ? String(comment.created_at) : null,
      updated_at: comment.updated_at ? String(comment.updated_at) : null
    }))

    return NextResponse.json(convertBigIntToString({
      comments: formattedComments
    }))
  } catch (error) {
    console.error("Error fetching mute comments:", error)
    return NextResponse.json(
      { error: "Error fetching comments" },
      { status: 500 }
    )
  }
}

export async function POST(request) {
  try {
    const cookieStore = await cookies()
    const { user, error: userError, status: userStatus } = getAuthenticatedUser(cookieStore)
    
    if (userError) {
      return NextResponse.json({ error: userError }, { status: userStatus })
    }

    const flags = await getUserFlags(user.steamId)
    if (!hasPermission(flags, "@web/mute.comment.add")) {
      return NextResponse.json(
        { error: "Access denied - @web/mute.comment.add required" },
        { status: 403 }
      )
    }

    const body = await request.json()
    const { muteId, comment } = body

    if (!muteId) {
      return NextResponse.json(
        { error: "Mute ID is required" },
        { status: 400 }
      )
    }

    if (!comment || !comment.trim()) {
      return NextResponse.json(
        { error: "Comment is required" },
        { status: 400 }
      )
    }

    const mute = await db.query(
      `SELECT id, player_name, player_steamid, admin_steamid FROM sa_mutes WHERE id = ?`,
      [muteId]
    )

    if (!mute || mute.length === 0) {
      return NextResponse.json(
        { error: "Mute not found" },
        { status: 404 }
      )
    }

    const adminQuery = await db.query(
      `SELECT player_name FROM sa_admins WHERE player_steamid = ? LIMIT 1`,
      [user.steamId]
    )
    const adminName = adminQuery && adminQuery.length > 0 ? adminQuery[0].player_name : "Admin Web"

    const result = await db.query(
      `INSERT INTO sp_mute_comments (mute_id, admin_steamid, admin_name, comment)
       VALUES (?, ?, ?, ?)`,
      [muteId, user.steamId, adminName, comment.trim()]
    )

    const commentId = Number(result.insertId)

    try {
      const muteData = {
        id: muteId,
        playerName: mute[0].player_name,
        playerSteamId: mute[0].player_steamid
      }

      const adminData = {
        name: adminName,
        steamId: user.steamId
      }

      await sendDiscordWebhook('create', 'mute_comment', {
        ...muteData,
        commentId: commentId,
        comment: comment.trim()
      }, adminData)
    } catch (webhookError) {
      console.error("Error sending webhook (comment created anyway):", webhookError)
    }

    return NextResponse.json({
      success: true,
      message: "Comment added successfully",
      commentId: commentId
    })
  } catch (error) {
    console.error("Error creating mute comment:", error)
    return NextResponse.json(
      { error: "Error creating comment" },
      { status: 500 }
    )
  }
}

export async function PATCH(request) {
  try {
    const cookieStore = await cookies()
    const { user, error: userError, status: userStatus } = getAuthenticatedUser(cookieStore)
    
    if (userError) {
      return NextResponse.json({ error: userError }, { status: userStatus })
    }

    const body = await request.json()
    const { id, comment } = body

    if (!id) {
      return NextResponse.json(
        { error: "Comment ID is required" },
        { status: 400 }
      )
    }

    if (!comment || !comment.trim()) {
      return NextResponse.json(
        { error: "Comment is required" },
        { status: 400 }
      )
    }

    const existingComment = await db.query(
      `SELECT admin_steamid, mute_id FROM sp_mute_comments WHERE id = ?`,
      [id]
    )

    if (!existingComment || existingComment.length === 0) {
      return NextResponse.json(
        { error: "Comment not found" },
        { status: 404 }
      )
    }

    if (String(existingComment[0].admin_steamid) !== String(user.steamId)) {
      return NextResponse.json(
        { error: "You can only edit your own comments" },
        { status: 403 }
      )
    }

    await db.query(
      `UPDATE sp_mute_comments SET comment = ?, updated_at = NOW() WHERE id = ?`,
      [comment.trim(), id]
    )

    const mute = await db.query(
      `SELECT id, player_name, player_steamid FROM sa_mutes WHERE id = ?`,
      [existingComment[0].mute_id]
    )

    const adminQuery = await db.query(
      `SELECT player_name FROM sa_admins WHERE player_steamid = ? LIMIT 1`,
      [user.steamId]
    )
    const adminName = adminQuery && adminQuery.length > 0 ? adminQuery[0].player_name : "Admin Web"

    try {
      const muteData = {
        id: existingComment[0].mute_id,
        playerName: mute[0]?.player_name || "Unknown",
        playerSteamId: mute[0]?.player_steamid || "Unknown"
      }

      const adminData = {
        name: adminName,
        steamId: user.steamId
      }

      await sendDiscordWebhook('update', 'mute_comment', {
        ...muteData,
        commentId: id,
        comment: comment.trim()
      }, adminData)
    } catch (webhookError) {
      console.error("Error sending webhook (comment updated anyway):", webhookError)
    }

    return NextResponse.json({
      success: true,
      message: "Comment updated successfully"
    })
  } catch (error) {
    console.error("Error updating mute comment:", error)
    return NextResponse.json(
      { error: "Error updating comment" },
      { status: 500 }
    )
  }
}

export async function DELETE(request) {
  try {
    const cookieStore = await cookies()
    const { user, error: userError, status: userStatus } = getAuthenticatedUser(cookieStore)
    
    if (userError) {
      return NextResponse.json({ error: userError }, { status: userStatus })
    }

    const { searchParams } = new URL(request.url)
    const id = searchParams.get("id")

    if (!id) {
      return NextResponse.json(
        { error: "Comment ID is required" },
        { status: 400 }
      )
    }

    const existingComment = await db.query(
      `SELECT admin_steamid, mute_id, comment FROM sp_mute_comments WHERE id = ?`,
      [id]
    )

    if (!existingComment || existingComment.length === 0) {
      return NextResponse.json(
        { error: "Comment not found" },
        { status: 404 }
      )
    }

    const flags = await getUserFlags(user.steamId)
    const commentAdminSteamId = String(existingComment[0].admin_steamid)
    const currentUserSteamId = String(user.steamId)

    const canDelete = hasPermission(
      flags,
      "@web/mute.comment.delete",
      true,
      commentAdminSteamId,
      currentUserSteamId
    )

    if (!canDelete) {
      return NextResponse.json(
        { error: "Access denied - You don't have permission to delete this comment" },
        { status: 403 }
      )
    }

    const mute = await db.query(
      `SELECT id, player_name, player_steamid FROM sa_mutes WHERE id = ?`,
      [existingComment[0].mute_id]
    )

    const adminQuery = await db.query(
      `SELECT player_name FROM sa_admins WHERE player_steamid = ? LIMIT 1`,
      [user.steamId]
    )
    const adminName = adminQuery && adminQuery.length > 0 ? adminQuery[0].player_name : "Admin Web"

    try {
      const muteData = {
        id: existingComment[0].mute_id,
        playerName: mute[0]?.player_name || "Unknown",
        playerSteamId: mute[0]?.player_steamid || "Unknown"
      }

      const adminData = {
        name: adminName,
        steamId: user.steamId
      }

      await sendDiscordWebhook('delete', 'mute_comment', {
        ...muteData,
        commentId: id,
        comment: existingComment[0].comment
      }, adminData)
    } catch (webhookError) {
      console.error("Error sending webhook (comment deleted anyway):", webhookError)
    }

    await db.query(`DELETE FROM sp_mute_comments WHERE id = ?`, [id])

    return NextResponse.json({
      success: true,
      message: "Comment deleted successfully"
    })
  } catch (error) {
    console.error("Error deleting mute comment:", error)
    return NextResponse.json(
      { error: "Error deleting comment" },
      { status: 500 }
    )
  }
}