...
import { syncServerGroupAdmins, syncAllServersAdmins } from "@/lib/admin-sync"
...
export async function PATCH(request) {
  try {
    ...
    if (serverIds !== undefined) {
      ...
      const addedServerIds = serverIds.map(id => Number(id)).filter(id => !oldServerIds.includes(id))
      if (addedServerIds.length > 0) {
        try {
          await syncAllServersAdmins(addedServerIds)
        } catch (syncAllError) {
          console.error("Error syncing all servers admins:", syncAllError)
        }
      }
    }
