  ...
  try {
    ...
    if (user.steamId) {
      try {
        ...
        if (playerProfile) {
          profile = {
            displayName: playerProfile.displayName || "User",
    ...
    if (user.steamId) {
      try {
        ...
        console.error("Error verifying admin flags:", error)
        return NextResponse.json(
          { error: "Error verifying permissions", message: error.message, user: null, isAdmin: false, flags: [] },
          { status: 500 },
        )
      }
    }
