...
const TabsList = ({ children, activeTab, setActiveTab }) => {
  ...
  const columnCount = validChildren.length;
  
  return (
    <div className="grid w-full bg-zinc-900 border border-zinc-800 rounded-lg p-1 mb-6" style={{ gridTemplateColumns: `repeat(${columnCount}, 1fr)` }}>
      {Children.map(children, (child) => {
        if (!isValidElement(child)) return null;
        return cloneElement(child, { activeTab, setActiveTab });
      })}
...
const TabsTrigger = ({ value, children, activeTab, setActiveTab }) => (
  <button onClick={() => setActiveTab(value)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === value ? '' : 'text-zinc-300 hover:text-zinc-100 hover:bg-zinc-800'}`}
